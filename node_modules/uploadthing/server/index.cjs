...
const conditionalDevServer = (presigned, apiKey)=>{
    return Effect__namespace.gen(function*() {
        const file = yield* shared.fetchEff(shared.generateUploadThingURL(`/v6/pollUpload/${presigned.key}`)).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(PollUploadResponse)), Effect__namespace.andThen((res)=>res.status === "done" && res.fileData ? Effect__namespace.succeed(res.fileData) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
            while: (err)=>err instanceof shared.RetryError,
            schedule: Schedule__namespace.exponential(10, 4).pipe(// 10ms, 40ms, 160ms, 640ms...
            Schedule__namespace.union(Schedule__namespace.spaced(1000)), Schedule__namespace.compose(Schedule__namespace.elapsed), Schedule__namespace.whileOutput(Duration__namespace.lessThanOrEqualTo(Duration__namespace.minutes(1))))
        }), Effect__namespace.catchTag("RetryError", ()=>new shared.UploadThingError({
                code: "UPLOAD_FAILED",
        ...
        if (isValidResponse(callbackResponse)) {
            yield* Effect__namespace.logInfo("Successfully simulated callback for file", presigned.key);
        } else {
...
const uploadMultipart = (file, presigned)=>Effect__namespace.gen(function*() {
        yield* Effect__namespace.logDebug(`Uploading file ${file.name} with ${presigned.urls.length} chunks of size ${presigned.chunkSize} bytes each`);
        const etags = yield* Effect__namespace.forEach(presigned.urls, (url, index)=>{
            ...
            }).pipe(Effect__namespace.andThen((etag)=>({
                    tag: etag,
                    partNumber: index + 1
        ...
        yield* Effect__namespace.logDebug("File", file.name, "uploaded successfully.");
        yield* Effect__namespace.logDebug("Completing multipart upload...");
        yield* completeMultipartUpload(presigned, etags);
        yield* Effect__namespace.logDebug("Multipart upload complete.");
    });
/**
 * Used by server uploads where progress is not needed.
 * Uses normal fetch API.
 */ const uploadPart = (opts)=>shared.fetchEff(opts.url, {
        method: "PUT",
        body: opts.chunk,
        headers: {
    ...
    }).pipe(Effect__namespace.andThen((res)=>res.ok && res.headers.get("Etag") ? Effect__namespace.succeed(res.headers.get("Etag")) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
        while: (res)=>res instanceof shared.RetryError,
        schedule: Schedule__namespace.exponential(Duration__namespace.millis(10), 4).pipe(// 10ms, 40ms, 160ms, 640ms...
        Schedule__namespace.andThenEither(Schedule__namespace.spaced(Duration__namespace.seconds(1))), Schedule__namespace.compose(Schedule__namespace.elapsed), Schedule__namespace.whileOutput(Duration__namespace.lessThanOrEqualTo(Duration__namespace.minutes(1)))),
        times: opts.maxRetries
    }), Effect__namespace.tapErrorTag("RetryError", ()=>// Max retries exceeded, tell UT server that upload failed
        abortMultipartUpload({
            key: opts.key,
            uploadId: opts.uploadId
        }).pipe(Effect__namespace.andThen((res)=>{
            new shared.UploadThingError({
                code: "UPLOAD_FAILED",
                message: `Failed to upload file ${opts.fileName} to S3`,
                cause: res
            });
        }))));
const completeMultipartUpload = (presigned, etags)=>shared.fetchEff(shared.generateUploadThingURL("/v6/completeMultipart"), {
        method: "POST",
        body: JSON.stringify({
...
const abortMultipartUpload = (presigned)=>shared.fetchEff(shared.generateUploadThingURL("/v6/failureCallback"), {
        method: "POST",
        body: JSON.stringify({
            fileKey: presigned.key,
            uploadId: presigned.uploadId
...
const parseAndValidateRequest = (input, opts, adapter)=>Effect__namespace.gen(function*() {
        ...
        if (action && !types_cjs.isActionType(action)) {
            const msg = `Expected ${types_cjs.VALID_ACTION_TYPES.map((x)=>`"${x}"`).join(", ").replace(/,(?!.*,)/, " or")} but got "${action}"`;
        ...
        if (hook && !types_cjs.isUploadThingHook(hook)) {
            const msg = `Expected ${types_cjs.VALID_UT_HOOKS.map((x)=>`"${x}"`).join(", ").replace(/,(?!.*,)/, " or")} but got "${hook}"`;
...
const buildRequestHandler = (opts, adapter)=>(input)=>handleRequest.pipe(Effect__namespace.provideServiceEffect(RequestInput, parseAndValidateRequest(input, opts, adapter)), Effect__namespace.catchTags({
            InvalidJson: (e)=>new shared.UploadThingError({
                    code: "INTERNAL_SERVER_ERROR",
                    message: "An error occured while parsing input/output",
                    cause: e
                }),
            BadRequestError: (e)=>new shared.UploadThingError({
                    code: "INTERNAL_SERVER_ERROR",
...
const handleCallbackRequest = Effect__namespace.gen(function*() {
    ...
    yield* shared.fetchEff(shared.generateUploadThingURL("/v6/serverCallback"), {
        method: "POST",
...
const handleUploadAction = Effect__namespace.gen(function*() {
    ...
    yield* assertFilesMeetConfig(files, parsedConfig).pipe(Effect__namespace.mapError((e)=>new shared.UploadThingError({
            code: "BAD_REQUEST",
    ...
    const presignedUrls = yield* shared.fetchEff(shared.generateUploadThingURL("/v6/prepareUpload"), {
        method: "POST",
        body: JSON.stringify({
    ...
    if (opts.isDev) {
        ...
        promise = Effect__namespace.forEach(presignedUrls, (presigned)=>conditionalDevServer(presigned, opts.apiKey).pipe(Effect__namespace.either), {
            concurrency: 10
...
const uploadPresignedPost = (file, presigned)=>Effect__namespace.gen(function*() {
        yield* Effect__namespace.logDebug(`Uploading file ${file.name} using presigned POST URL`);
        const formData = new FormData();
        Object.entries(presigned.fields).forEach(([k, v])=>formData.append(k, v));
        formData.append("file", file); // File data **MUST GO LAST**
        const res = yield* shared.fetchEff(presigned.url, {
            method: "POST",
            body: formData,
            headers: new Headers({
                Accept: "application/xml"
            })
        }).pipe(Effect__namespace.tapErrorCause(()=>shared.fetchEff(shared.generateUploadThingURL("/v6/failureCallback"), {
                method: "POST",
                body: JSON.stringify({
                    fileKey: presigned.key,
                    uploadId: null
            ...
            }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(FailureCallbackResponse)))));
        if (!res.ok) {
            const text = yield* Effect__namespace.promise(res.text);
            yield* Effect__namespace.logError(`Failed to upload file ${file.name} to presigned POST URL. Response: ${text}`);
            return yield* new shared.UploadThingError({
                code: "UPLOAD_FAILED",
...
const uploadFilesInternal = (input)=>getPresignedUrls(input).pipe(Effect__namespace.andThen((presigneds)=>Effect__namespace.forEach(presigneds, (file)=>uploadFile(file).pipe(Effect__namespace.tapError((error)=>Effect__namespace.logError("Upload failed:", error)), Effect__namespace.match({
                onFailure: (error)=>({
                        data: null,
 ...
 */ const downloadFiles = (urls, downloadErrors)=>Effect__namespace.forEach(urls, (_url, idx)=>Effect__namespace.gen(function*() {
            ...
            return yield* Effect__namespace.promise(()=>response.blob()).pipe(Effect__namespace.andThen((blob)=>new UTFile([
                    blob
                ], name, {
                    customId
                })));
...
const getPresignedUrls = (input)=>Effect__namespace.gen(function*() {
        ...
        yield* Effect__namespace.logDebug("Getting presigned URLs for files", fileData);
        const responseSchema = S__namespace.Struct({
            data: PresignedURLResponse
        });
        const presigneds = yield* shared.fetchEff(shared.generateUploadThingURL("/v6/uploadFiles"), {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                files: fileData,
        ...
        }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(responseSchema)), Effect__namespace.catchTag("ParseError", (e)=>Effect__namespace.die(e)), Effect__namespace.catchTag("FetchError", (e)=>Effect__namespace.die(e)));
        yield* Effect__namespace.logDebug("Got presigned URLs:", presigneds.data);
...
const uploadFile = (input)=>Effect__namespace.gen(function*() {
        ...
        yield* shared.fetchEff(shared.generateUploadThingURL(`/v6/pollUpload/${presigned.key}`)).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(PollUploadResponse)), Effect__namespace.tap(Effect__namespace.logDebug("Polled upload", presigned.key)), Effect__namespace.andThen((res)=>res.status === "done" ? Effect__namespace.succeed(undefined) : Effect__namespace.fail(new shared.RetryError())), Effect__namespace.retry({
            while: (err)=>err instanceof shared.RetryError,
            schedule: Schedule__namespace.exponential(Duration__namespace.millis(10), 4).pipe(// 10ms, 40ms, 160ms, 640ms...
            Schedule__namespace.andThenEither(Schedule__namespace.spaced(Duration__namespace.seconds(1))), Schedule__namespace.compose(Schedule__namespace.elapsed), Schedule__namespace.whileOutput(Duration__namespace.lessThanOrEqualTo(Duration__namespace.minutes(1))))
        }), Effect__namespace.catchTag("RetryError", (e)=>Effect__namespace.die(e)));
...
class UTApi {
    constructor(opts){
        this.requestUploadThing = (pathname, body, responseSchema)=>{
            ...
            Effect__namespace.runSync(Effect__namespace.logDebug("Requesting UploadThing:", {
                url,
                body,
            ...
            return shared.fetchEff(url, {
                ...
                headers
            }).pipe(Effect__namespace.andThen(shared.parseResponseJson), Effect__namespace.andThen(S__namespace.decodeUnknown(responseSchema)), Effect__namespace.catchTag("FetchError", (err)=>Effect__namespace.logError("Request failed:", err).pipe(Effect__namespace.andThen(()=>Effect__namespace.die(err)))), Effect__namespace.catchTag("ParseError", (err)=>Effect__namespace.logError("Response parsing failed:", err).pipe(Effect__namespace.andThen(()=>Effect__namespace.die(err)))), Effect__namespace.tap((res)=>Effect__namespace.logDebug("UploadThing response:", res)));
        };
        this.executeAsync = (program, signal)=>program.pipe(withMinimalLogLevel(this.logLevel), Effect__namespace.provide(ConsolaLogger), Effect__namespace.provideService(shared.FetchContext, {
                fetch: this.fetch,
                baseHeaders: this.defaultHeaders
            }), (e)=>Effect__namespace.runPromise(e, signal ? {
                    signal
                } : undefined));
        /**
   ...
   */ this.updateACL = async (keys, acl, opts)=>{
    ...
    async uploadFiles(files, opts) {
        ...
        const uploads = await this.executeAsync(Effect__namespace.flatMap(uploadFilesInternal({
            ...
            acl: opts?.acl
        }), (ups)=>Effect__namespace.succeed(Array.isArray(files) ? ups : ups[0])).pipe(Effect__namespace.tap((res)=>Effect__namespace.logDebug("Finished uploading:", res))), opts?.signal);
        return uploads;
    }
    async uploadFilesFromUrl(urls, opts) {
        guardServerOnly();
        const downloadErrors = {};
        const uploads = await this.executeAsync(downloadFiles(shared.asArray(urls), downloadErrors).pipe(Effect__namespace.andThen((files)=>files.filter((f)=>f != null)), Effect__namespace.andThen((files)=>uploadFilesInternal({
                files,
                contentDisposition: opts?.contentDisposition ?? "inline",
                metadata: opts?.metadata ?? {},
                acl: opts?.acl
        ...
        Effect__namespace.runSync(Effect__namespace.logDebug("Finished uploading:", uploadFileResponse));
    ...
    const POST = async (request)=>{
        ...
        if (response.success === false) {
            return Response.json(formatError(response.error, opts.router), {
                status: shared.getStatusCodeFromError(response.error),
                headers: {
                    "x-uploadthing-version": version
                }
            });
