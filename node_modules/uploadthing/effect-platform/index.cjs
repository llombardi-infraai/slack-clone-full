...
const createRouteHandler = (opts)=>{
    ...
    const requestHandler = buildRequestHandler(opts, "effect-platform");
    const getBuildPerms = buildPermissionsInfoHandler(opts);
    const appendUploadThingResponseHeaders = platform.HttpMiddleware.make(Effect__namespace.map(platform.HttpServerResponse.setHeader("x-uploadthing-version", version)));
    return platform.HttpRouter.empty.pipe(platform.HttpRouter.get("/", platform.HttpServerResponse.json(getBuildPerms())), platform.HttpRouter.post("/", Effect__namespace.flatMap(platform.HttpServerRequest.HttpServerRequest, (req)=>requestHandler({
            /**
           ...
           */ req: Effect__namespace.if(req.source instanceof Request, {
                onTrue: ()=>Effect__namespace.succeed(req.source),
                onFalse: ()=>req.json.pipe(// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                    Effect__namespace.flatMap((body)=>toWebRequest(req.source, body)), Effect__namespace.catchTags({
                        RequestError: (error)=>new shared.UploadThingError({
                                code: "BAD_REQUEST",
        ...
        }).pipe(withMinimalLogLevel(opts.config?.logLevel), Effect__namespace.provide(ConsolaLogger), Effect__namespace.provideService(shared.FetchContext, {
        ...
        }), Effect__namespace.andThen((response)=>platform.HttpServerResponse.json(response.body)), Effect__namespace.catchTag("UploadThingError", (error)=>platform.HttpServerResponse.json(formatError(error, opts.router), {
                status: shared.getStatusCodeFromError(error)
            }))))), platform.HttpRouter.use(appendUploadThingResponseHeaders));
};
