import { HttpMiddleware, HttpServerResponse, HttpRouter, HttpServerRequest } from '@effect/platform';
import * as Effect from 'effect/Effect';
import { ValidContentDispositions, ValidACLs, fetchEff, generateUploadThingURL, parseResponseJson, RetryError, UploadThingError, signPayload, isObject, getTypeFromFileName, objectKeys, InvalidRouteConfigError, fileSizeToBytes, bytesToFileSize, FetchContext, getFullApiUrl, verifySignature, parseRequestJson, fillInputRouteConfig, getStatusCodeFromError } from '@uploadthing/shared';
...
const createRouteHandler = (opts)=>{
    ...
    const appendUploadThingResponseHeaders = HttpMiddleware.make(Effect.map(HttpServerResponse.setHeader("x-uploadthing-version", version)));
    return HttpRouter.empty.pipe(HttpRouter.get("/", HttpServerResponse.json(getBuildPerms())), HttpRouter.post("/", Effect.flatMap(HttpServerRequest.HttpServerRequest, (req)=>requestHandler({
            /**
           ...
           */ req: Effect.if(req.source instanceof Request, {
                onTrue: ()=>Effect.succeed(req.source),
                onFalse: ()=>req.json.pipe(// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                    Effect.flatMap((body)=>toWebRequest(req.source, body)), Effect.catchTags({
                        RequestError: (error)=>new UploadThingError({
                                code: "BAD_REQUEST",
        ...
        }).pipe(withMinimalLogLevel(opts.config?.logLevel), Effect.provide(ConsolaLogger), Effect.provideService(FetchContext, {
        ...
        }), Effect.andThen((response)=>HttpServerResponse.json(response.body)), Effect.catchTag("UploadThingError", (error)=>HttpServerResponse.json(formatError(error, opts.router), {
